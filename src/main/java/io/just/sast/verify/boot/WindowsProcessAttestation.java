...
/** Child-side native proof for the Windows AppContainer token. */
public final class WindowsProcessAttestation {
    ...
    public static boolean appContainerLow() {
        ...
        try {
            ...
            if (!api.OpenProcessToken(kernel.GetCurrentProcess(), TOKEN_QUERY, tokenRef)) {
                return false;
            }
            ...
            if (!api.GetTokenInformation(token, TOKEN_IS_APPCONTAINER, appContainer,
                    (int) appContainer.size(), returned) || appContainer.getInt(0) == 0) {
                return false;
            }
            ...
            if (!api.GetTokenInformation(token, TOKEN_INTEGRITY_LEVEL, label, size, returned)) {
                return false;
            }
            Pointer sid = label.getPointer(0);
            if (sid == null || Pointer.nativeValue(sid) == 0L) {
                return false;
            }
            ...
            if (subAuthorities == 0 || subAuthorities > 15) {
                return false;
            }
            ...
            if (token != null) {
                try {
                    Native.load("kernel32", KernelApi.class).CloseHandle(token);
                } catch (Throwable ignored) {
                    // Attestation already failed or the process is exiting.
                }
            }
        }
    }
