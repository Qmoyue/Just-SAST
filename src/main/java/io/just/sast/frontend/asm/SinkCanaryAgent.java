...
/**
 * Per-child sink agent. Boundary mode injects the historical Error canary. REAL_SANITIZED mode
 * injects a separate bootstrap gate and lets only a typed, fixed-argument sink call proceed.
 * The two paths intentionally do not share a positive event: a target body/call can therefore
 * never be reported merely because the old canary latch or a Just adapter fired.
 */
public final class SinkCanaryAgent {

    static final String GATE_INTERNAL = "io/just/sast/verify/boot/SinkCanaryGate";
    static final String EXEC_INTERNAL = "io/just/sast/verify/boot/SinkExecutionGate";
    private static final String REAL_MODE = "REAL_SANITIZED";

    public static void premain(String args, Instrumentation inst) {
        if (args == null) return;
        String[] parts = args.split("\\|", -1);
        if (parts.length < 9 || parts[3].isEmpty()) return;
        ...
        for (String name : sinks.keySet()) {
            // In REAL_SANITIZED the call site in the target class is the boundary.  Rewriting
            // the implementation of a JDK sink as well would instrument its private plumbing
            // (Runtime.exec -> ProcessImpl, reflection accessors, etc.) and could block the
            // very API whose successful call we are measuring.  Boundary mode still
            // retransforms platform sink classes because it must place the historical canary
            // at their entry/native boundary.
        ...
        try {
            ...
            // The probe fails closed when the attestation is absent.
        }
    }
    ...
    private static boolean configureExecution(String[] parts, int entryHash, String token) {
        String entryClass = parts[1].substring(0, entryHash);
        String entryTail = parts[1].substring(entryHash + 1);
        int descriptorHash = entryTail.indexOf('#');
        String entryMethod = descriptorHash < 0 ? entryTail : entryTail.substring(0, descriptorHash);
        String scratch = parts.length > 11 ? parts[11]
                : System.getProperty("just.verify.safe-scratch", "");
        String safeJava = parts.length > 12 ? parts[12]
                : System.getProperty("just.verify.safe-java", "");
        try {
            Class<?> gate = Class.forName(EXEC_INTERNAL.replace('/', '.'), true, null);
            gate.getDeclaredMethod("setExecution", String.class, String.class, String.class,
                            String.class, String.class, String.class, String.class, String.class)
                    .invoke(null, REAL_MODE, entryClass, entryMethod, parts[2], token,
                            scratch, parts.length > 13 ? parts[13] : "", safeJava);
            return Boolean.TRUE.equals(gate.getDeclaredMethod("configured").invoke(null));
        } catch (Throwable ignored) {
            return false;
        }
    ...
    private static Map<String, Set<String>> parseSinks(String encoded) {
        ...
        for (String spec : encoded.split(",", -1)) {
            ...
            result.computeIfAbsent(spec.substring(0, hash), ignored -> new HashSet<>())
                    .add(spec.substring(hash + 1));
        }
        return result;
    }
    ...
    private static Map<String, Set<String>> parseNativeIndex(String encoded) {
        Map<String, Set<String>> result = new HashMap<>();
        if (encoded == null || encoded.isEmpty() || encoded.length() > 64 * 1024) return result;
        for (String spec : encoded.split(",", -1)) {
            ...
            result.computeIfAbsent(spec.substring(0, first), ignored -> new HashSet<>())
                    .add(spec.substring(first + 1));
            if (result.size() > 256) break;
        }
        return result;
    }

    private static void appendBootstrap(Instrumentation inst, String bootJar) {
        try {
            inst.appendToBootstrapClassLoaderSearch(new java.util.jar.JarFile(bootJar));
        } catch (Exception ignored) {
            // The probe's authenticated readiness check reports a missing bootstrap gate.
        }
    }

    /** Bytecode transformer used by both production premain and direct contract tests. */
    public static class CanaryTransformer implements ClassFileTransformer {
        ...
        public CanaryTransformer(Map<String, Set<String>> sinks, String token,
            ...
            this.nativeMethodIndex = new ConcurrentHashMap<>();
            if (nativeIndex != null) {
                nativeIndex.forEach((owner, methods) -> {
                    if (owner != null && methods != null && !methods.isEmpty()) {
                        this.nativeMethodIndex.put(owner,
                                ConcurrentHashMap.newKeySet(methods.size()));
                        this.nativeMethodIndex.get(owner).addAll(methods);
                    }
                });
            }
        }
                ...
                reader.accept(new ClassVisitor(Opcodes.ASM9, writer) {
                    ...
                    public MethodVisitor visitMethod(int access, String name, String desc,
                        ...
                        MethodVisitor visitor = super.visitMethod(access, name, desc,
                                sig, exceptions);
                        boolean injectable = entryMethods != null && matches(entryMethods, name, desc)
                        ...
                        return new MethodVisitor(Opcodes.ASM9, visitor) {
                            ...
                            public void visitCode() {
                                super.visitCode();
                                if (injectable) {
                                    emitCanary(this, className + "#" + name + "#" + desc);
                                    changed[0] = true;
                                }
                            }
                            ...
                            public void visitMethodInsn(int opcode, String owner,
                                ...
                                if (callMethods != null && matches(callMethods, calledName, calledDesc)) {
        ...
        private byte[] realTransform(String className, byte[] bytes,
                                     Set<String> entryMethods) {
            boolean[] changed = {false};
            try {
                ClassReader reader = new ClassReader(bytes);
                ...
                reader.accept(new ClassVisitor(Opcodes.ASM9, writer) {
                    ...
                    public MethodVisitor visitMethod(int access, String name, String desc,
                        ...
                        if ((access & Opcodes.ACC_NATIVE) != 0) {
                            nativeMethodIndex.computeIfAbsent(className,
                        ...
                        boolean entry = className.equals(CanaryTransformer.this.entryClass)
                                ...
                                && (CanaryTransformer.this.entryDescriptor.isEmpty()
        ...
        private final class RealMethodVisitor extends AdviceAdapter {
            ...
            private RealMethodVisitor(MethodVisitor delegate, int access, String className,
            ...
            public void visitMethodInsn(int opcode, String owner, String name,
                ...
                // Only the selected sink is allowed to cross into an actual API.  Every other
                // dangerous capability encountered while the target body is running is a
                // nested effect and is stopped before the platform call.  Sanitizing a nested
                // call would still execute target-controlled control flow and would make a
                // positive sink result depend on an adapter side effect.
                ...
                if (!exactCall && "NESTED_BLOCK".equals(kind)
                    ...
                    // The corresponding NEW already emitted blockNested. Do not spill the
                    // uninitialized receiver into locals, which would make the class fail
                    // verification before the runtime guard can execute.
                ...
                if (nativeMethod(owner, name, desc)) {
                    super.visitMethodInsn(opcode, owner, name, desc, isInterface);
                    emitExecution("nativeCall", owner + "#" + name + "#" + desc);
                    changed[0] = true;
                    return;
                }
                super.visitMethodInsn(opcode, owner, name, desc, isInterface);
            ...
            private void sanitizeEntryArguments() {
                if ("APPLICATION_BODY".equals(realKind)) {
                    ...
                    for (int i = 0; i < args.length; i++) {
                        sanitizeApplicationArgument(args[i], i);
                    }
                } else if ("RUNTIME_EXEC".equals(realKind)) {
                    Type[] args = Type.getArgumentTypes(methodDesc);
                    if (args.length > 0) {
                        loadArg(0);
                        String helper = args[0].getSort() == Type.ARRAY
                                ? "safeCommandArray" : "safeCommand";
                        String descriptor = args[0].getSort() == Type.ARRAY
                                ? "([Ljava/lang/String;Ljava/lang/String;)[Ljava/lang/String;"
                                : "(Ljava/lang/String;Ljava/lang/String;)Ljava/lang/String;";
                        emitSanitizer(helper, descriptor);
                        storeArg(0);
                    }
                    for (int i = 1; i < args.length; i++) {
                        if (args[i].getSort() == Type.ARRAY
                                && "java/lang/String".equals(args[i].getElementType().getInternalName())) {
                            loadArg(i);
                            emitSanitizer("safeEnvironment",
                                    "([Ljava/lang/String;Ljava/lang/String;)[Ljava/lang/String;");
                            storeArg(i);
                        } else if (args[i].getSort() == Type.OBJECT
                                && "java/io/File".equals(args[i].getInternalName())) {
                            loadArg(i);
                            emitSanitizer("safeWorkingDirectory",
                    ...
                    loadThis();
                    emitSanitizer("safeProcessBuilder",
                            "(Ljava/lang/ProcessBuilder;Ljava/lang/String;)Ljava/lang/ProcessBuilder;");
                    pop();
                }
            }
            ...
            private void rewriteCall(int opcode, String owner, String name, String desc,
                ...
                int receiver = -1;
                if (opcode != Opcodes.INVOKESTATIC && !constructor) {
                    receiver = newLocal(Type.getObjectType(owner));
                    storeLocal(receiver);
                }
                sanitizeCallArguments(kind, name, args, locals);
                if ("NESTED_BLOCK".equals(kind)) {
                    emitExecution("blockNested", "NESTED");
                }
                if (receiver >= 0) {
                    loadLocal(receiver);
                    sanitizeReceiver(kind);
                    if ("URL_LOOPBACK".equals(kind) || "SOCKET_LOOPBACK".equals(kind)) {
                        storeLocal(receiver);
                        loadLocal(receiver);
                    }
                }
                ...
                super.visitMethodInsn(opcode, owner, actualName, desc, isInterface);
                if (exactCall) {
                    emitExecution("afterCall", owner + "#" + name + "#" + desc);
                }
            ...
            private void sanitizeApplicationArgument(Type type, int index) {
                ...
                if (type.getSort() == Type.OBJECT
                    ...
                    switch (type.getSort()) {
                        case Type.BOOLEAN -> { helper = "safeBoolean"; descriptor = "(ZLjava/lang/String;)Z"; }
                        case Type.BYTE -> { helper = "safeByte"; descriptor = "(BLjava/lang/String;)B"; }
                        case Type.SHORT -> { helper = "safeShort"; descriptor = "(SLjava/lang/String;)S"; }
                        case Type.CHAR -> { helper = "safeChar"; descriptor = "(CLjava/lang/String;)C"; }
                        case Type.INT -> { helper = "safeInt"; descriptor = "(ILjava/lang/String;)I"; }
                        case Type.FLOAT -> { helper = "safeFloat"; descriptor = "(FLjava/lang/String;)F"; }
                        case Type.LONG -> { helper = "safeLong"; descriptor = "(JLjava/lang/String;)J"; }
                        case Type.DOUBLE -> { helper = "safeDouble"; descriptor = "(DLjava/lang/String;)D"; }
            ...
            private void sanitizeCallArguments(String kind, String name, Type[] args, int[] locals) {
                ...
                if ("CLASS_FOR_NAME".equals(kind) && args.length > 0) {
                    loadLocal(locals[0]);
                    emitSanitizer("safeClassName",
                            "(Ljava/lang/String;Ljava/lang/String;)Ljava/lang/String;");
                    storeLocal(locals[0]);
                    if (args.length == 3) {
                        loadLocal(locals[1]);
                        emitSanitizer("safeBoolean", "(ZLjava/lang/String;)Z");
                        storeLocal(locals[1]);
                        loadLocal(locals[2]);
                        emitSanitizer("safeClassLoader",
                                "(Ljava/lang/ClassLoader;Ljava/lang/String;)Ljava/lang/ClassLoader;");
                        storeLocal(locals[2]);
                    }
                } else if ("METHOD_INVOKE".equals(kind) && args.length == 2) {
                    loadLocal(locals[0]);
                    emitSanitizer("safeInvocationTarget",
                            "(Ljava/lang/Object;Ljava/lang/String;)Ljava/lang/Object;");
                    storeLocal(locals[0]);
                    loadLocal(locals[1]);
                    emitSanitizer("safeArguments",
                            "([Ljava/lang/Object;Ljava/lang/String;)[Ljava/lang/Object;");
                    storeLocal(locals[1]);
                    ...
                    if (first.getSort() == Type.OBJECT
                            && "java/nio/file/Path".equals(first.getInternalName())) {
                        emitSanitizer("safePath",
                                "(Ljava/nio/file/Path;Ljava/lang/String;)Ljava/nio/file/Path;");
                    } else if (first.getSort() == Type.OBJECT
                            && "java/io/File".equals(first.getInternalName())) {
                        emitSanitizer("safeFile", "(Ljava/io/File;Ljava/lang/String;)Ljava/io/File;");
                    } else {
                        emitSanitizer("safeFilePath",
                                "(Ljava/lang/String;Ljava/lang/String;)Ljava/lang/String;");
                    }
                    storeLocal(locals[0]);
                    for (int i = 1; i < args.length; i++) {
                        if (args[i].getSort() == Type.BOOLEAN) {
                            push(false);
                            storeLocal(locals[i]);
                        } else if (args[i].getSort() == Type.ARRAY
                                && "java/nio/file/OpenOption".equals(args[i].getElementType().getInternalName())) {
                            loadLocal(locals[i]);
                            emitSanitizer("safeOpenOptions",
                                    "([Ljava/nio/file/OpenOption;Ljava/lang/String;)[Ljava/nio/file/OpenOption;");
                            storeLocal(locals[i]);
                        }
                    }
                } else if ("SOCKET_LOOPBACK".equals(kind) && args.length > 0) {
                    loadLocal(locals[0]);
                    emitSanitizer("safeSocketAddress",
                            "(Ljava/lang/Object;Ljava/lang/String;)Ljava/net/InetSocketAddress;");
                    storeLocal(locals[0]);
                    if (args.length > 1 && args[1].getSort() == Type.INT) {
                        push(50);
                        storeLocal(locals[1]);
                    }
                }
                if ("NATIVE_FIXTURE".equals(kind) && args.length == 1) {
                    loadLocal(locals[0]);
                    String helper = "loadLibrary".equals(name)
                            ? "safeNativeLibraryName" : "rewriteNativeLoad";
                    emitSanitizer(helper,
                            "(Ljava/lang/String;Ljava/lang/String;)Ljava/lang/String;");
                    storeLocal(locals[0]);
                }
            }
            ...
            private void sanitizeReceiver(String kind) {
                if ("PROCESS_BUILDER_START".equals(kind)) {
                    emitSanitizer("safeProcessBuilder",
                            "(Ljava/lang/ProcessBuilder;Ljava/lang/String;)Ljava/lang/ProcessBuilder;");
                } else if ("CLASS_NEW_INSTANCE".equals(kind)) {
                    emitSanitizer("safeClass", "(Ljava/lang/Object;Ljava/lang/String;)Ljava/lang/Class;");
                } else if ("METHOD_INVOKE".equals(kind)) {
                    emitSanitizer("safeMethod", "(Ljava/lang/Object;Ljava/lang/String;)Ljava/lang/reflect/Method;");
                } else if ("CONSTRUCTOR_NEW_INSTANCE".equals(kind)) {
                    emitSanitizer("safeConstructor",
                            "(Ljava/lang/Object;Ljava/lang/String;)Ljava/lang/reflect/Constructor;");
                } else if ("URL_LOOPBACK".equals(kind)) {
                    emitSanitizer("safeUrl", "(Ljava/lang/Object;Ljava/lang/String;)Ljava/net/URL;");
                } else if ("SOCKET_LOOPBACK".equals(kind)) {
                    emitSanitizer("safeSocket", "(Ljava/lang/Object;Ljava/lang/String;)Ljava/net/Socket;");
                }
            }

            private void emitExecution(String method, String spec) {
                if (spec == null) {
                    visitLdcInsn(token);
                    super.visitMethodInsn(Opcodes.INVOKESTATIC, EXEC_INTERNAL, method,
            ...
            private void emitSanitizer(String name, String descriptor) {
                visitLdcInsn(token);
                super.visitMethodInsn(Opcodes.INVOKESTATIC, EXEC_INTERNAL, name, descriptor, false);
            }
        }

        private static boolean matches(Set<String> methods, String name, String desc) {
            return methods != null && (methods.contains(name) || methods.contains(name + "#" + desc));
        }
        ...
        private boolean nativeMethod(String owner, String name, String desc) {
            ...
            if (methods != null && methods.contains(signature)) return true;
            return false;
        }
        ...
        private void emitCanary(MethodVisitor visitor, String spec) {
            visitor.visitLdcInsn(spec);
            visitor.visitLdcInsn(token);
            visitor.visitMethodInsn(Opcodes.INVOKESTATIC, GATE_INTERNAL, "hit",
                    "(Ljava/lang/String;Ljava/lang/String;)V", false);
        }
        ...
        private static String nestedKind(String owner, String name, String desc) {
            if (owner.equals("java/lang/System") && (name.equals("load") || name.equals("loadLibrary"))
                    && "(Ljava/lang/String;)V".equals(desc)) return "NESTED_BLOCK";
            ...
            if (owner.equals("java/lang/ProcessBuilder") && name.equals("start")
                    && "()Ljava/lang/Process;".equals(desc)) return "NESTED_BLOCK";
            if (owner.equals("java/lang/Class") && name.equals("forName")
                    && ("(Ljava/lang/String;)Ljava/lang/Class;".equals(desc)
                    || "(Ljava/lang/String;ZLjava/lang/ClassLoader;)Ljava/lang/Class;".equals(desc))) return "NESTED_BLOCK";
            if (owner.equals("java/lang/Class") && name.equals("newInstance")
                    && "()Ljava/lang/Object;".equals(desc)) return "NESTED_BLOCK";
            if (owner.equals("java/lang/reflect/Method") && name.equals("invoke")
                    && "(Ljava/lang/Object;[Ljava/lang/Object;)Ljava/lang/Object;".equals(desc)) return "NESTED_BLOCK";
            if (owner.equals("java/lang/reflect/Constructor") && name.equals("newInstance")
                    && "([Ljava/lang/Object;)Ljava/lang/Object;".equals(desc)) return "NESTED_BLOCK";
            if ((owner.equals("java/nio/file/Files") && name.equals("newOutputStream")
                    && "(Ljava/nio/file/Path;[Ljava/nio/file/OpenOption;)Ljava/io/OutputStream;".equals(desc))
                    || ((owner.equals("java/io/FileOutputStream") || owner.equals("java/io/FileWriter"))
                    && name.equals("<init>") && fileConstructorDescriptor(desc))) return "NESTED_BLOCK";
            if (owner.equals("java/net/URL") && (name.equals("openConnection") || name.equals("openStream"))
                    && ("()Ljava/net/URLConnection;".equals(desc) || "()Ljava/io/InputStream;".equals(desc))) return "NESTED_BLOCK";
            if (owner.equals("java/net/Socket") && name.equals("connect")
                    && ("(Ljava/net/SocketAddress;)V".equals(desc) || "(Ljava/net/SocketAddress;I)V".equals(desc))) return "NESTED_BLOCK";
            if (isNestedDangerous(owner, name)) return "NESTED_BLOCK";
            return "";
        }
        ...
        private static boolean isNestedDangerous(String owner, String name) {
            if (owner.equals("java/lang/System") && (name.equals("exit") || name.equals("halt")
                    || name.equals("setSecurityManager"))) return true;
            if (owner.equals("java/lang/Runtime") && (name.equals("exec") || name.equals("load")
                    || name.equals("loadLibrary") || name.equals("halt"))) return true;
            if (owner.equals("java/lang/ClassLoader")
                    && (name.equals("defineClass") || name.equals("loadClass"))) return true;
            if (owner.startsWith("javax/naming/") || owner.startsWith("java/rmi/")) {
                return name.equals("lookup") || name.equals("list") || name.equals("bind")
                        || name.equals("connect") || name.equals("accept");
            }
            if (owner.startsWith("javax/script/") && (name.equals("eval")
                    || name.equals("getEngineByName"))) return true;
            if (owner.startsWith("java/lang/invoke/") && name.startsWith("invoke")) return true;
            if (owner.equals("java/sql/Statement") || owner.equals("java/sql/Connection")
                    || owner.equals("java/net/URLClassLoader")) return true;
            if (owner.equals("java/nio/file/Files")) {
                return name.startsWith("write") || name.startsWith("delete")
                        || name.startsWith("move") || name.startsWith("copy")
                        || name.startsWith("create") || name.startsWith("set")
                        || name.equals("newByteChannel");
            }
            if (owner.equals("java/io/File") && (name.equals("delete")
                    || name.equals("deleteOnExit") || name.equals("createNewFile")
                    || name.equals("mkdir") || name.equals("mkdirs")
                    || name.equals("renameTo"))) return true;
            if (owner.equals("java/io/RandomAccessFile") && name.equals("<init>")) return true;
            if (owner.equals("java/net/URLConnection") || owner.equals("java/net/HttpURLConnection")) {
                return name.equals("connect") || name.equals("getInputStream")
                        || name.equals("getOutputStream") || name.equals("getResponseCode");
            }
            if (owner.equals("java/net/Socket") && (name.equals("connect")
                    || name.equals("getInputStream") || name.equals("getOutputStream"))) return true;
            if (owner.equals("java/net/ServerSocket") || owner.equals("java/net/DatagramSocket")) {
                return name.equals("<init>") || name.equals("bind") || name.equals("accept")
                        || name.equals("connect") || name.equals("send");
            }
            if (owner.equals("java/net/InetAddress") && name.startsWith("getBy")) return true;
            if (owner.startsWith("java/net/http/") && (name.equals("send")
                    || name.equals("sendAsync"))) return true;
            return owner.startsWith("javax/xml/transform/") || owner.startsWith("javax/el/")
                    || owner.startsWith("org/apache/commons/jexl/")
                    || owner.startsWith("groovy/");
        }

        private static boolean commandDescriptor(String desc) {
            return desc.endsWith(")Ljava/lang/Process;")
                    && (desc.startsWith("(Ljava/lang/String;") || desc.startsWith("([Ljava/lang/String;"));
        }

        private static boolean fileConstructorDescriptor(String desc) {
            return "(Ljava/lang/String;)V".equals(desc) || "(Ljava/io/File;)V".equals(desc)
                    || "(Ljava/lang/String;Z)V".equals(desc) || "(Ljava/io/File;Z)V".equals(desc);
        }
    }
}
